...
if dye == 0:
    while True:
        try:
            ...
            dye = int(raw_input("How many sides does your dye have: d"))
            break
        except ValueError:
...
while True:
    try:
        # Note: Python 2.x users should use raw_input, the equivalent of 3.x's input
        rolls = rolls + int(raw_input("How many times do you want to roll? (recommended 5 * the dye size, ie 200 for a d20): "))
        break
    except ValueError:
        print("Invalid dye side. Try again. \n.")
        #better try again... Return to the start of the loop
    ...
    while True:
        try:
            ...
            if randomInputs:
                ...
                val = int(raw_input("Enter your value: "))
        except ValueError:
            print("Invalid dye side. Try again. \n.")
            #better try again... Return to the start of the loop
            continue
...
while True:
    try:
        # Note: Python 2.x users should use raw_input, the equivalent of 3.x's input
        saveTally = raw_input("Save tally? [y/n/o]: ")
        if(saveTally == "n"):
            saveTally = False
            break
        elif(saveTally == "y"):
